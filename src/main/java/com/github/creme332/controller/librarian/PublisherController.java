...
public class PublisherController {
    ...
    private void savePublisher() {
        ...
        if (publisher.getName().length() == 0 || publisher.getEmail().length() == 0 || publisher.getCountry().length() == 0)
            return;

        try {
            Publisher.save(publisher);
            JOptionPane.showMessageDialog(publisherForm, "Publisher saved successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
